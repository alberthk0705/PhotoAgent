    ...
    >
      ...
      >
        <div className="mb-3 flex items-center gap-3">
          <h2 className="text-xs font-semibold text-neutral-100">Choose a head to frame</h2>
          <span className="text-[11px] text-neutral-500">
            {status === 'loading' && 'Loading the detector and scanning…'}
            {status === 'error' && <span className="text-red-400">{error}</span>}
            {status === 'done' && (heads.length ? `${heads.length} found` : 'No heads found in this photo')}
          </span>
          <button
            onClick={onClose}
          ...
          <div className="checkerboard relative" style={{ width: dispW, height: dispH }}>
            ...
            {status === 'loading' && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                <span className="rounded bg-neutral-900 px-3 py-1.5 text-[11px] text-neutral-300">Scanning…</span>
              </div>
            )}
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <p className="text-[10px] leading-snug text-neutral-600">
            Click a box to centre that head in the cell. Detection runs on your device.
          </p>
        </div>
      </div>
    </div>
